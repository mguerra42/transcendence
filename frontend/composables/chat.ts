...
export class WrappedConversation {
    ...
    leave(cb?: Function)
    {
        ...
        this.socket.emit('conversations:leave', { channelId: this.channelId }, (answer) => {
    ...
    syncMessages() {
        ...
        this.socket.emit(`conversations:messages`, { channelId: this.channelId, readUntil: this.readUntil, }, (answer) => {
    ...
    saveSettings() {
        ...
        this.socket.emit(`conversations:update`, { 
            channelId: this.channelId, 
            name: this.channel.name,
            description: this.channel.description,
            type: this.channel.type,
            password: this.channel.type == 'PROTECTED' ? this.channel.password : undefined,
         }, (answer) => {
    ...
    searchUser(search: string) {
        return this.users.filter(u => u.user.username.toLowerCase().includes(search.toLowerCase()))
    }
    ...
    write()
    {
        ...
        this.socket.emit(`conversations:message`, { channelId: this.channelId, message: this.message }, (answer) => {
...
export const useChat = defineStore('chat', () => {
    ...
    const _onNotification = ({type, message} : {type?: string, message: string}) => {
    ...
    const isActiveConversation = (conversation: WrappedConversation) => activeConversation.value === conversation
    ...
    const _onConversationsList = ({conversations: _conversations} : {conversations: Conversation[]}) => {
        console.log("conversations", _conversations)
        _conversations.map(c => _onConversationsSync({conversation: c}))
    }

    const _onConversationsSync = ({conversation: _conversation, show = false} : {conversation: Conversation, show?: boolean}) => {
    ...
    const _onConversationsMessage = ({channelId, id, from, content, timestamp} : {
        id: number,
        channelId: number,
        from: number,
        content: string,
    ...
    const showProfile = (user) => {
        ...
        currentProfile.value = {user, online: false}
        //setView('profile')
    }
