...
const conversationsArray = computed(() =>
  [...chat.conversations].map((data) => data[1])
);
const channels = computed(() =>
  conversationsArray.value
    .filter(
      (conversation: WrappedConversation) => conversation.channel.type != "DM"
    )
    .sort(
      (a: WrappedConversation, b: WrappedConversation) =>
        b.channel.id - a.channel.id
    )
);
const dms = computed(() =>
  conversationsArray.value
...
<template>
  ...
  >
    <div
      ...
      class="b-blue b-1 md:hidden h-50px w-10px absolute top-[calc(50%-25px)] px-6px cursor-pointer hover:bg-blue hover:text-white right-0 rounded-l flex flex-col justify-center items-center z-999 bg-zinc-700"
    >
      <div class="i-mdi:chevron-left"></div>
    </div>
