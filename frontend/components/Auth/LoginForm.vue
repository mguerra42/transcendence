...
<template>
    <div class="flex flex-rows-5 gap-5 p-6 relative">
        ...
        <h1 class="home-font text-4xl">Log in to your account</h1>
        <form @submit.prevent="client.auth.authenticateUser({
            ...
            <div>
                ...
                <input type="email" v-model="email" name="email" class="px-3 text-2xl text-black outline-0 w-80" style="background-color: transparent; border:none; caret-color:rgb(0, 255, 191);"/>
            </div>
            <div style="display: flex; align-items: baseline;">
                <span class="home-font text-2xl">Password:</span>
                <input type="password" v-model="password" name="password"
                    class="px-3 text-2xl text-black outline-0" style="background-color: transparent; caret-color:rgb(0, 255, 191);"/>
            </div>
            <div class="text-red">
                {{ auth.error }}
            </div>
            ...
            <!-- Default Login-->
            <button class="home-font text-2xl cursor-pointer hover:scale-105 transition" :class="{
            ...
            <!-- Google Login-->
            <button type="button" @click="loginGoogle" class="text-2xl border-gray-300 text-gray-600 font-semibold rounded-lg cursor-pointer hover:scale-105 transition px-4 py-2 flex">
            ...
            <a href="#" class="home-font text-2xl flex" @click="auth.mode = 'signup'">
                Sign up
            </a>
        </form>
    </div>
